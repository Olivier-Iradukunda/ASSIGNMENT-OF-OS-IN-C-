...
/**
  ...
  *...Using the priority (non-preemptive) Scheduling Algorithm
  */
int main() {
	int i, j, zero_found , processes_count, p, invalid_priority;
	i = j = zero_found = processes_count = p = invalid_priority = 0;
	double total = 0.0, accumulated_time = 0, arrival, burst, high_priority = DBL_MAX, arrived_first = DBL_MAX;
	vector<double> arrival_time, burst_time, waiting_times, priority;

	cout<<"Howdy!!!"<<endl;
	cout<<"* This program helps you to compute the average waiting time of a number of processes ..."<<endl;
	cout<<" ... in a priority scheduling 'non_preemptive' mode."<<endl;
	...
	while(arrival_time.size() > 0) {
		...
		while(i < arrival_time.size()) {
			if ((arrival_time[i] <= accumulated_time) && (priority[i] <= high_priority)) {
                    if((priority[i] == high_priority) && (arrival_time[i] >= arrived_first)) {
    ...
    cout<<"Individual waiting times are: ";
